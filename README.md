...
## Artisan (ARAREプロジェクト用 改造版)
通常のArtisanに開発課題用に独自の改造を加えたバージョンです。
CO2や湿度のデータをLCDやグラフで表示して可視化できるように変更しています。

## ダウンロード方法と起動方法
1. 画面右側の「Releases」から最新版のzipファイルをダウンロードしてください。
2. zipファイルを解凍する前に、右クリックして「プロパティ」を開き、セキュリティの「許可する（ブロック解除）」にチェックしてください。
3. 解凍後、フォルダ内の `dist` の中にある `Artisan_Arare.exe` をダブルクリックして起動してください。

## Artisanを起動した後の設定
今回追加した機能を使うには、Arduino + TC4シールドと改造したライブラリが必要です。

1. Artisanが起動したら、メニューの `Config` から `Device` を開き `TC4` を選択する。
2. その後 `Extra Devices` タブを選択し、デバイスを1つ追加する。
3. プルダウンを開き、`Arduino TC4 34` を選択する。
4. Label 1に `arare co2` などの名前を付ける。
   **（※ `arare` という文字をLabel 1の最初に入力することで本システムが動作します）**
5. 任意の色を選択し、OKを押す。
6. その後、シリアル通信の設定を行えば設定完了です。

## プログラムの主な変更点
* **comm.py の ARDUINOTC4tenprature メソッド**
  * TC4からのカンマ区切りの値を `res` リストに格納されるように変更。
  * `res` リストの後ろから2つの値（CO2とHum）を抜き出して `self.aw.shread` 変数に格納。
* **canvas.py の sample メソッド**
  * `self.aw.shread` 変数を参照して、Extra Deviceの既存のグラフ描画機能に乗せるように変更。
